...
GradientBouyancyResults calculateBouyancy(double x, double y, double z, double roll, double pitch, double yaw){
    ...
    vector<double> bounding_box = {.3503, .3863, .4337, .4299, .1791, .1829};
    vector<Vertex> raw_vertices = generateRawVertices(bounding_box);

    //Make this out of one for right now - fraction of volume
    double bouyant_force = 1;
    ...
    Vector3d orentation = Vector3d(roll,pitch,yaw);

    GradientBouyancyResults bouyancyResults;
    calculateBouyancy(raw_vertices, orentation, position, bouyant_density, bouyant_force, &(bouyancyResults.force), &(bouyancyResults.location));
...
void calculateBoundingPaths(vector<Vector3d> points, vector<Vector3d> *upper_bound, vector<Vector3d> *lower_bound){
    ...
    if ((points.at(size(points) - 1).x - points.at(0).x) == 0){
        //if vertical
        midline_slope = 999999999;
    }
    else{
        midline_slope = (points.at(size(points) - 1).y - points.at(0).y) / (points.at(size(points) - 1).x - points.at(0).x);
    }

    while ((size(*lower_bound) + size(*upper_bound)) != size(points)){
        //points above midline go in upper bound below go in
        if(points.at(counter).y > points.at(0).y + midline_slope * (points.at(counter).x - points.at(0).x)){
...
vector<double> pointsToPlane(Vector3d p1, Vector3d p2, Vector3d p3){
    ...
    Vector3d v1 = p1.SubtractVector(p2);
    Vector3d v2 = p1.SubtractVector(p3);

    Vector3d cp = v1.CrossVector(v2);
    double k = cp.DotVector(p1);

    vector<double> plane={cp.x, cp.y, cp.z, k};
    return plane;
}
...
bool determineAdditiveArea(vector<double> plane, Vector3d other_surfaced_vertice){
    ...
    double planarZ = (plane[3] - plane[0] * other_surfaced_vertice.x - plane[1] * other_surfaced_vertice.y) / plane[2];

    if(planarZ > other_surfaced_vertice.z){
        return true;
    } else {
        return false;
    }
...
bool determineIfAdditiveArea(Vector3d izv1, Vector3d izv2, Vector3d izi1, Vector3d izi2, Vector3d izo1, Vector3d izo2){
    ...
    if ((izv2.x - izv1.x) != 0){
       slope = (izv2.y - izv1.y) / (izv2.x - izv1.x);
    }
    else if (((izi1.x - izi2.x) == 0) and (izv1.x == izi1.x)){
        // if the line is vertical
        //if the second line is vertical too and shares the same x coordinate, the volume of the iz will be zero 
        return true;
    }

    double y_intercept = izv1.y - (slope * izv1.x);

    //determine if the intersections are above the line
    if((izi1.y > (izi1.x * slope + y_intercept)) && (izi2.y > (izi2.x * slope + y_intercept))){
        ...
        if ((izo1.y > (izo1.x * slope + y_intercept)) && (izo2.y > (izo2.x * slope + y_intercept))){
            //negative area
            return false;
        }else if((izo1.y < (izo1.x * slope + y_intercept)) && (izo2.y < (izo2.x * slope + y_intercept))){
            //additive area
            return true;
        }else{
            // cout << "Determine additive area: This shouldn't happen... 1" << endl;
        }
    }else if((izi1.y < (izi1.x * slope + y_intercept)) && (izi2.y < (izi2.x * slope + y_intercept))){
...
vector<Vertex> findOtherVertices(vector<Vertex> surfaced_vertices, vector<Vertex>vertices){
    ...
    for(int counter = 0; counter < size(surfaced_vertices_locations); counter++){
        ...
        for(int inner_counter = 0; inner_counter < size(vertices_locations); inner_counter++){
            if(vertex.IsEqual(vertices_locations.at(inner_counter))){
                vertex_found = true;
            }
        }
...
void sumBouyantForces(vector<double> forces, vector<Vector3d> centroids, double *sum_forces, Vector3d *overall_centroid){
    //  z position of bouyant forces dont mattertan
   ...
   // find the total torque around the x-axis
    ...
    for(int counter = 0; counter < size(centroids); counter++){
        x_torque += centroids[counter].y * forces[counter];
    }

    //find the total torque around the y-axis
    double y_torque = 0;
    for(int counter = 0; counter < size(centroids); counter++){
        y_torque += centroids[counter].x * forces[counter]; 
    }
...
void calculateBouyancy(vector<Vertex> raw_vertices, Vector3d euler_rotation, Vector3d position, double bouyant_density, double bouyant_force, double *calculated_force, Vector3d *acting_location){
    ...
    Matrix3d rotation_matrix = rotationFromEuler(euler_rotation.x, euler_rotation.y, euler_rotation.z);
    ...
    Vector3d relative_postion_cob_vector(
        (raw_vertices[0].vector_from_center.x + raw_vertices[7].vector_from_center.x) / 2,
        (raw_vertices[0].vector_from_center.y + raw_vertices[7].vector_from_center.y) / 2,
        (raw_vertices[0].vector_from_center.z + raw_vertices[7].vector_from_center.z) / 2);

    Vector3d absolute_postion_cob_vector = rotation_matrix.premultiplyVector(relative_postion_cob_vector);
    Vector3d cov = absolute_postion_cob_vector.AddVector(position);
    ...
    if(size(surfaced_vertices) == 4){
        ...
        vector<Vector3d> points = {
            Vector3d(surfaced_vertices.at(0).location.x, surfaced_vertices.at(0).location.y, 0),
            Vector3d(surfaced_vertices.at(1).location.x, surfaced_vertices.at(1).location.y, 0),
            Vector3d(surfaced_vertices.at(2).location.x, surfaced_vertices.at(2).location.y, 0),
            Vector3d(surfaced_vertices.at(3).location.x, surfaced_vertices.at(3).location.y, 0)
            };
        ...
        if(submerged && (surfaced_vertices.at(0).location.x == surfaced_vertices.at(0).intersections[0].x) and (surfaced_vertices.at(0).location.y == surfaced_vertices.at(0).intersections[0].y)){
        ...
        else if((surfaced_vertices.at(0).location.x == surfaced_vertices.at(0).intersections[0].x) && (surfaced_vertices.at(0).location.y == surfaced_vertices.at(0).intersections[0].y)){            
        ...
        vector<Vector3d> points2 = {
            Vector3d(surfaced_segments[0].vertex_one.location.x, surfaced_segments[0].vertex_one.location.y, 0), 
            Vector3d(surfaced_segments[0].vertex_two.location.x, surfaced_segments[0].vertex_two.location.y, 0),
        ...
        vector<Vector3d> points3 = {
            Vector3d(surfaced_segments[1].vertex_one.location.x, surfaced_segments[1].vertex_one.location.y, 0), 
            Vector3d(surfaced_segments[1].vertex_two.location.x, surfaced_segments[1].vertex_two.location.y, 0),
        ...
        vector<Vector3d> points4 = {
            Vector3d(surfaced_segments[2].vertex_one.location.x, surfaced_segments[2].vertex_one.location.y, 0), 
            Vector3d(surfaced_segments[2].vertex_two.location.x, surfaced_segments[2].vertex_two.location.y, 0),
        ...
        vector<Vector3d> points5 = {
            Vector3d(surfaced_segments[3].vertex_one.location.x, surfaced_segments[3].vertex_one.location.y, 0), 
            Vector3d(surfaced_segments[3].vertex_two.location.x, surfaced_segments[3].vertex_two.location.y, 0),
        ...
        //rectangle 1
        points = {
            Vector3d(double_intersection_vertices[0].location.x, double_intersection_vertices[0].location.y, 0), 
            Vector3d(single_intersection_vertex.location.x, single_intersection_vertex.location.y, 0), 
        ...
        //rectangle 2
        points = {
            Vector3d(double_intersection_vertices[1].location.x, double_intersection_vertices[1].location.y, 0), 
            Vector3d(single_intersection_vertex.location.x, single_intersection_vertex.location.y, 0), 
            single_intersection_vertex.intersections[0],
        ...
        vector<Vector3d> points = {
            Vector3d(location_zero.x, location_zero.y, 0), 
        ...
        points = {
            Vector3d(location_one.x, location_one.y, 0), 
        ...
        vector<Vector3d> points1 = {Vector3d(center_vertex.x, center_vertex.y, 0), surfaced_point_1, surfaced_point_2};
        calculateBoundingPaths(points1, &u1, &l1);
        vector<double> p1 = pointsToPlane(center_vertex, surfaced_point_1, surfaced_point_2);
        
        vector<Vector3d> points2 = {Vector3d(center_vertex.x, center_vertex.y, 0), surfaced_point_1, surfaced_point_3};
        calculateBoundingPaths(points2, &u2, &l2);
        vector<double> p2 = pointsToPlane(center_vertex, surfaced_point_1, surfaced_point_3);

        vector<Vector3d> points3 = {Vector3d(center_vertex.x, center_vertex.y, 0), surfaced_point_3, surfaced_point_2};
        calculateBoundingPaths(points3, &u3, &l3);
...
Matrix3d rotationFromEuler(double x, double y, double z){
    ...
    Matrix3d matrix = Matrix3d(a,b,c,d,e,f,g,h,i);
