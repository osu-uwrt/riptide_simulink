...
void Do_Model0_step(void)
{
   ...
   */
  ct[2] = 340.03943213999992 * Do_Model0_U.GradientBouyancyForce;
  ...
  for (k = 0; k <= 4; k += 2) {
    ...
    tmp_4 = _mm_loadu_pd(&st_0[k]);
    _mm_storeu_pd(&rtb_ElementProduct_f[k], _mm_mul_pd(tmp_1, tmp_4));

    /* Product: '<S15>/Element Product' incorporates:
     *  Product: '<S46>/Element Product'
     */
    tmp_1 = _mm_loadu_pd(&tmp_0[k]);
    tmp_4 = _mm_loadu_pd(&ct_0[k]);
    _mm_storeu_pd(&rtb_ElementProduct_bv[k], _mm_mul_pd(tmp_1, tmp_4));
  ...
  rtb_y = Do_Model0_U.PreviousAngularVelcoity[2];

  /* End of Outputs for SubSystem: '<Root>/Do_Model' */
  for (k = 0; k <= 0; k += 2) {
    /* Outputs for Atomic SubSystem: '<Root>/Do_Model' */
    /* Product: '<S5>/Apply Rotation' */
    tmp_1 = _mm_loadu_pd(&rtb_rotation_b[k]);
    tmp_4 = _mm_loadu_pd(&rtb_rotation_b[k + 3]);
    tmp_2 = _mm_loadu_pd(&rtb_rotation_b[k + 6]);
    _mm_storeu_pd(&rtb_column_m[k], _mm_add_pd(_mm_mul_pd(tmp_2, _mm_set1_pd
      (rtb_y)), _mm_add_pd(_mm_mul_pd(tmp_4, _mm_set1_pd(rtb_Abs3)), _mm_add_pd
      (_mm_mul_pd(tmp_1, _mm_set1_pd(rtb_Abs1)), _mm_set1_pd(0.0)))));
  ...
  for (k = 0; k <= 0; k += 2) {
    ...
    /* Product: '<S9>/Convert to World Frame 1' */
    _mm_storeu_pd(&rtb_SumMoments[k], _mm_add_pd(_mm_mul_pd(tmp_2, _mm_set1_pd
      (rtb_column_m[2])), _mm_add_pd(_mm_mul_pd(tmp_4, _mm_set1_pd(rtb_column_m
      [1])), _mm_add_pd(_mm_mul_pd(tmp_1, _mm_set1_pd(rtb_column_m[0])),
                        _mm_set1_pd(0.0)))));
  ...
  for (k = 0; k <= 0; k += 2) {
    ...
    tmp_1 = _mm_loadu_pd(&rtb_rotation_g[k]);
    tmp_4 = _mm_loadu_pd(&rtb_rotation_g[k + 3]);
    tmp_2 = _mm_loadu_pd(&rtb_rotation_g[k + 6]);

    /* Product: '<S8>/Apply Rotation' */
    _mm_storeu_pd(&rtb_ApplyInverseRoation[k], _mm_add_pd(_mm_mul_pd(tmp_2,
      _mm_set1_pd(rtb_y)), _mm_add_pd(_mm_mul_pd(tmp_4, _mm_set1_pd(rtb_Abs3)),
      _mm_add_pd(_mm_mul_pd(tmp_1, _mm_set1_pd(rtb_Abs1)), _mm_set1_pd(0.0)))));
  ...
  for (k = 0; k <= 0; k += 2) {
    ...
    /* Product: '<S9>/Convert to World Frame' */
    _mm_storeu_pd(&rtb_Product[k], _mm_add_pd(_mm_mul_pd(tmp_2, _mm_set1_pd(st[2])),
      _mm_add_pd(_mm_mul_pd(tmp_4, _mm_set1_pd(st[1])), _mm_add_pd(_mm_mul_pd
      (tmp_1, _mm_set1_pd(st[0])), _mm_set1_pd(0.0)))));
