...
IntegrationZone::IntegrationZone(vector<Vector3d> upper, vector<Vector3d> lower, vector<double>plane, double bouyantDensity){
    ...
    for(int counter = 0; counter < size(upper); counter++){
        ...
        while(inner_counter < size(lower) - 1){
            ...
            if(segment.isInXBound(point.x)){
                Vector3d additional_point;
                segment.returnInterpolatedPoint(point.x, &additional_point);
...
Vector3d IntegrationZone::pointsToMxPlusB(Vector3d p1, Vector3d p2){
    //[m,b] - m is slope, b is y intercept
    double m = (p1.y - p2.y) / (p1.x - p2.x);
    double b = p1.y - (p1.x * m);

    return Vector3d(m, b, 0);
}
...
void IntegrationZone::integrate(){
    ...
    while((counter + upper_vertical < (size(this->upper_bound) - 1)) && (counter + lower_vertical < (size(this->lower_bound) - 1))){
        ...
        while(abs(this->upper_bound[upper_vertical + counter].x - this->upper_bound[upper_vertical + 1 + counter].x) < 2.7e-10){
        ...
        while(abs(this->lower_bound[lower_vertical + counter].x - this->lower_bound[lower_vertical + counter + 1].x) < 2.7e-10){
            //if the width of the zone is small that that of a water molecule, its essentially vertical (this is done for a reason)
            lower_vertical += 1;
        ...
        Vector3d high_line = this->pointsToMxPlusB(this->upper_bound[counter + upper_vertical], this->upper_bound[counter + upper_vertical + 1]);
        Vector3d low_line = this->pointsToMxPlusB(this->lower_bound[counter + lower_vertical], this->lower_bound[counter + lower_vertical + 1]);

        double volume = this->integrate_eq(high_line.x, low_line.x, high_line.y, low_line.y, 
            this->upper_bound[counter + upper_vertical + 1].x, this->upper_bound[counter + upper_vertical].x, 
        ...
        Vector3d centroid = this->calculateCentroid(high_line.x, low_line.x, high_line.y, low_line.y, 
            this->upper_bound[counter + upper_vertical + 1].x, this->upper_bound[counter + upper_vertical].x, 
            this->plane_coefficents[3], this->plane_coefficents[0], this->plane_coefficents[1], this->plane_coefficents[2],
            volume);
