...
void ThrusterSolver::ThrusterSolver_inv(const real_T x[64], real_T y[64])
{
  ...
  for (c_j = 0; c_j < 7; c_j++) {
    ...
    n = 8 - c_j;
    jA = 0;
    ThrusterSolver_B.smax_c = std::abs(ThrusterSolver_B.A[jj]);
    for (d_k = 2; d_k <= n; d_k++) {
      ThrusterSolver_B.s_f = std::abs(ThrusterSolver_B.A[(jj + d_k) - 1]);
      if (ThrusterSolver_B.s_f > ThrusterSolver_B.smax_c) {
        jA = d_k - 1;
        ThrusterSolver_B.smax_c = ThrusterSolver_B.s_f;
      }
    }
    ...
    if (ThrusterSolver_B.A[jj + jA] != 0.0) {
      if (jA != 0) {
        ...
        for (jA = 0; jA < 8; jA++) {
          kAcol = jA << 3;
          d_k = kAcol + c_j;
          ThrusterSolver_B.smax_c = ThrusterSolver_B.A[d_k];
          kAcol += n;
          ThrusterSolver_B.A[d_k] = ThrusterSolver_B.A[kAcol];
          ThrusterSolver_B.A[kAcol] = ThrusterSolver_B.smax_c;
        }
      }
    ...
    jA = jj + 10;
    for (d_k = 0; d_k <= n; d_k++) {
      ThrusterSolver_B.smax_c = ThrusterSolver_B.A[((d_k << 3) + jj) + 8];
      if (ThrusterSolver_B.smax_c != 0.0) {
        kAcol = (jA - c_j) + 6;
        for (int32_T ijA{jA}; ijA <= kAcol; ijA++) {
          ThrusterSolver_B.A[ijA - 1] += ThrusterSolver_B.A[((jj + ijA) - jA) +
            1] * -ThrusterSolver_B.smax_c;
        }
      }
  ...
  for (n = 0; n < 8; n++) {
    ...
    for (d_k = 7; d_k >= 0; d_k--) {
      kAcol = d_k << 3;
      c_j = d_k + jA;
      ThrusterSolver_B.smax_c = y[c_j];
      if (ThrusterSolver_B.smax_c != 0.0) {
        y[c_j] = ThrusterSolver_B.smax_c / ThrusterSolver_B.A[d_k + kAcol];
...
{
  static const char_T tmp[16]{ '/', 't', 'h', 'r', 'u', 's', 't', 'e', 'r', '_',
    'f', 'o', 'r', 'c', 'e', 's' };
...
void ThrusterSolver::step()
{
  ...
  if (!p) {
    ...
    for (ThrusterSolver_B.k = 0; ThrusterSolver_B.k < 5; ThrusterSolver_B.k++) {
      ...
      ThrusterSolver_B.smax = std::abs(ThrusterSolver_B.A_c[ThrusterSolver_B.jj]);
      for (ThrusterSolver_B.e_k = 2; ThrusterSolver_B.e_k <=
           ThrusterSolver_B.value_tmp; ThrusterSolver_B.e_k++) {
        ThrusterSolver_B.s = std::abs(ThrusterSolver_B.A_c[(ThrusterSolver_B.jj
      ...
      for (ThrusterSolver_B.e_k = 0; ThrusterSolver_B.e_k <=
           ThrusterSolver_B.value_tmp; ThrusterSolver_B.e_k++) {
        ThrusterSolver_B.s = ThrusterSolver_B.A_c[(ThrusterSolver_B.e_k * 6 +
        ...
        if (ThrusterSolver_B.s != 0.0) {
          ...
          for (ThrusterSolver_B.ijA = ThrusterSolver_B.jA; ThrusterSolver_B.ijA <=
            ...
            ThrusterSolver_B.A_c[ThrusterSolver_B.ijA - 1] +=
              ...
              - ThrusterSolver_B.jA) + 1] * -ThrusterSolver_B.s;
          }
        }
    ...
    for (ThrusterSolver_B.value_tmp = 0; ThrusterSolver_B.value_tmp < 6;
      ...
      for (ThrusterSolver_B.e_k = 5; ThrusterSolver_B.e_k >= 0;
        ...
        ThrusterSolver_B.s = ThrusterSolver_B.b[ThrusterSolver_B.k];
        if (ThrusterSolver_B.s != 0.0) {
          ThrusterSolver_B.b[ThrusterSolver_B.k] = ThrusterSolver_B.s /
  ...
  ParamGet_ThrusterSolver_147.getParameter(&ThrusterSolver_B.value_g);
  for (ThrusterSolver_B.k = 0; ThrusterSolver_B.k <= 6; ThrusterSolver_B.k += 2)
  {
    ...
    _mm_storeu_pd
      (&ThrusterSolver_B.IndividualOveruseMutliplier[ThrusterSolver_B.k],
       _mm_sub_pd(_mm_set1_pd(static_cast<real_T>(ThrusterSolver_B.value_g)),
                  tmp));
  }
  ...
  ParamGet_ThrusterSolver_148.getParameter(&ThrusterSolver_B.value_g);
  ...
  for (ThrusterSolver_B.k = 0; ThrusterSolver_B.k < 8; ThrusterSolver_B.k++) {
    ...
    ThrusterSolver_B.s += ThrusterSolver_B.value_k[ThrusterSolver_B.k];
  ...
  ThrusterSolver_B.s = static_cast<real_T>(ThrusterSolver_B.value_g) -
    ThrusterSolver_B.s;
  ...
  ThrusterSolver_B.smax = ThrusterSolver_B.s + ThrusterSolver_P.Constant1_Value;
...
void ThrusterSolver::initialize()
{
  ...
  {
    static const char_T tmp[22]{ 't', 'a', 'l', 'o', 's', '_', 't', 'h', 'r',
      'u', 's', 't', 'e', 'r', '_', 'w', 'e', 'i', 'g', 'h', 't', 's' };

    static const char_T tmp_0[15]{ 't', 'a', 'l', 'o', 's', '_', 'w', 'r', 'e',
      'n', 'c', 'h', 'm', 'a', 't' };

    static const char_T tmp_1[15]{ 't', 'a', 'l', 'o', 's', '_', 'i', 'n', 'd',
      'i', 'v', '_', 'l', 'i', 'm' };

    static const char_T tmp_2[13]{ 't', 'a', 'l', 'o', 's', '_', 's', 'y', 's',
      '_', 'l', 'i', 'm' };
    ...
    for (int32_T i{0}; i < 22; i++) {
      ThrusterSolver_B.prmName[i] = tmp[i];
    }

    ThrusterSolver_B.prmName[22] = '\x00';
    varargin_1 = 0.0;
    ParamGet_ThrusterSolver_146.initParam(&ThrusterSolver_B.prmName[0]);
    ...
    for (int32_T i{0}; i < 15; i++) {
      ThrusterSolver_B.prmName_p[i] = tmp_0[i];
    }
    ...
    for (int32_T i{0}; i < 15; i++) {
      ThrusterSolver_B.prmName_p[i] = tmp_1[i];
    }
    ...
    prmName[13] = '\x00';
    ParamGet_ThrusterSolver_148.initParam(&prmName[0]);
