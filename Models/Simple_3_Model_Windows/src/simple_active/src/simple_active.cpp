...
void simple_active::step()
{
  ...
  simple_active_B.euler_tmp = simple_active_B.y[1] * simple_active_B.y[1];
  simple_active_B.euler_tmp_f = simple_active_B.y[2] * simple_active_B.y[2];
  simple_active_B.euler_tmp_g = simple_active_B.y[3] * simple_active_B.y[3];
  simple_active_B.euler[0] = rt_atan2d_snf((simple_active_B.y[1] *
    simple_active_B.y[2] + simple_active_B.y[0] * simple_active_B.y[3]) * 2.0,
    ((simple_active_B.Divide + simple_active_B.euler_tmp) -
     simple_active_B.euler_tmp_f) - simple_active_B.euler_tmp_g);
  ...
  simple_active_B.euler[2] = rt_atan2d_snf((simple_active_B.y[2] *
    ...
    ((simple_active_B.Divide - simple_active_B.euler_tmp) -
     simple_active_B.euler_tmp_f) + simple_active_B.euler_tmp_g);
  simple_active_B.trueCount = 0;
  if (mask1) {
    simple_active_B.trueCount = 1;
  }

  x_size = simple_active_B.trueCount;
  if (simple_active_B.trueCount - 1 >= 0) {
    simple_active_B.x_data = simple_active_B.aSinInput;
  }

  for (k = 0; k < simple_active_B.trueCount; k++) {
  ...
  simple_active_B.e_size[0] = 1;
  simple_active_B.e_size[1] = k;
  if (mask1) {
    f_data = 1;
  }

  if (simple_active_B.trueCount == k) {
    if (simple_active_B.trueCount - 1 >= 0) {
    ...
    simple_active_binary_expand_op(simple_active_B.euler, &f_data,
      &simple_active_B.x_data, &x_size, simple_active_B.y,
      simple_active_B.e_size);
  }
  ...
  simple_active_B.Divide = 1.0 / sqrt(((simple_active_B.In1_c.position.z *
    simple_active_B.In1_c.position.z + simple_active_B.In1_c.orientation.w *
    simple_active_B.In1_c.orientation.w) + simple_active_B.In1_c.position.x *
    simple_active_B.In1_c.position.x) + simple_active_B.In1_c.position.y *
    simple_active_B.In1_c.position.y);
  simple_active_B.y[0] = simple_active_B.In1_c.position.z *
    simple_active_B.Divide;
  simple_active_B.y[1] = simple_active_B.In1_c.orientation.w *
    simple_active_B.Divide;
  simple_active_B.y[2] = simple_active_B.In1_c.position.x *
    simple_active_B.Divide;
  simple_active_B.y[3] = simple_active_B.In1_c.position.y *
    simple_active_B.Divide;
  simple_active_B.aSinInput = (simple_active_B.y[1] * simple_active_B.y[3] -
    simple_active_B.y[0] * simple_active_B.y[2]) * -2.0;
  mask1 = (simple_active_B.aSinInput >= 0.99999999999999778);
  ...
  simple_active_B.e_size_c[0] = 1;
  simple_active_B.e_size_c[1] = k;
  if (mask1) {
    f_data_0 = 1;
  }
  ...
  simple_active_B.dv[0] = simple_active_B.In1.pose.pose.position.x;
  simple_active_B.dv[1] = simple_active_B.In1.pose.pose.position.y;
  simple_active_B.dv[2] = simple_active_B.In1.pose.pose.position.z;
  simple_active_B.dv[3] = simple_active_B.euler[0];
  simple_active_B.dv[4] = simple_active_B.euler[1];
  simple_active_B.dv[5] = simple_active_B.euler[2];
  simple_active_B.ControlSum[0] = simple_active_B.In1_c.orientation.x;
  simple_active_B.ControlSum[1] = simple_active_B.In1_c.orientation.y;
  simple_active_B.ControlSum[2] = simple_active_B.In1_c.orientation.z;
  simple_active_B.ControlSum[3] = simple_active_B.euler_p[0];
  simple_active_B.ControlSum[4] = simple_active_B.euler_p[1];
  simple_active_B.ControlSum[5] = simple_active_B.euler_p[2];
  for (simple_active_B.trueCount = 0; simple_active_B.trueCount <= 4;
       simple_active_B.trueCount += 2) {
    __m128d tmp;
    __m128d tmp_0;

    // Sum: '<S2>/Current Error'
    tmp = _mm_loadu_pd(&simple_active_B.dv[simple_active_B.trueCount]);
    tmp_0 = _mm_loadu_pd(&simple_active_B.ControlSum[simple_active_B.trueCount]);
    _mm_storeu_pd(&simple_active_B.CurrentError[simple_active_B.trueCount],
  ...
  if (rtIsNaN(simple_active_B.CurrentError[3])) {
    simple_active_B.Divide = (rtNaN);
  } else if (rtIsInf(simple_active_B.CurrentError[3])) {
    simple_active_B.Divide = (rtNaN);
  } else if (simple_active_B.CurrentError[3] == 0.0) {
    simple_active_B.Divide = 0.0;
  } else {
    simple_active_B.Divide = fmod(simple_active_B.CurrentError[3], 360.0);
    if (simple_active_B.Divide == 0.0) {
      simple_active_B.Divide = 0.0;
    } else if (simple_active_B.CurrentError[3] < 0.0) {
      simple_active_B.Divide += 360.0;
    }
  }

  simple_active_B.normalizedError[3] = simple_active_B.Divide - 180.0;
  if (rtIsNaN(simple_active_B.CurrentError[4])) {
    simple_active_B.Divide = (rtNaN);
  } else if (rtIsInf(simple_active_B.CurrentError[4])) {
    simple_active_B.Divide = (rtNaN);
  } else if (simple_active_B.CurrentError[4] == 0.0) {
    simple_active_B.Divide = 0.0;
  } else {
    simple_active_B.Divide = fmod(simple_active_B.CurrentError[4], 360.0);
    if (simple_active_B.Divide == 0.0) {
      simple_active_B.Divide = 0.0;
    } else if (simple_active_B.CurrentError[4] < 0.0) {
      simple_active_B.Divide += 360.0;
  ...
  simple_active_B.normalizedError[4] = simple_active_B.Divide - 180.0;
  if (rtIsNaN(simple_active_B.CurrentError[5])) {
    simple_active_B.Divide = (rtNaN);
  } else if (rtIsInf(simple_active_B.CurrentError[5])) {
    simple_active_B.Divide = (rtNaN);
  } else if (simple_active_B.CurrentError[5] == 0.0) {
    simple_active_B.Divide = 0.0;
  } else {
    simple_active_B.Divide = fmod(simple_active_B.CurrentError[5], 360.0);
    if (simple_active_B.Divide == 0.0) {
      simple_active_B.Divide = 0.0;
    } else if (simple_active_B.CurrentError[5] < 0.0) {
      simple_active_B.Divide += 360.0;
  ...
  simple_active_B.normalizedError[5] = simple_active_B.Divide - 180.0;
  ...
  ParamGet_simple_active_43.getParameter(&simple_active_B.value_g);

  // MATLABSystem: '<Root>/Get Parameter1'
  ParamGet_simple_active_44.getParameter(&simple_active_B.value_m);
  ...
  simple_active_B.Divide = static_cast<real_T>(simple_active_B.value_g) /
    static_cast<real_T>(simple_active_B.value_m);
  ...
  for (simple_active_B.trueCount = 0; simple_active_B.trueCount < 6;
       simple_active_B.trueCount++) {
    // MATLAB Function: '<S2>/Manage Accumulated Error' incorporates:
    //   Memory: '<S2>/Accumulated Error'

    simple_active_B.euler_tmp =
      simple_active_B.normalizedError[simple_active_B.trueCount];
    simple_active_B.euler_tmp_f =
      simple_active_DW.AccumulatedError_PreviousInput[simple_active_B.trueCount]
      + simple_active_B.euler_tmp;
    if (simple_active_B.euler_tmp > simple_active_B.Divide) {
      simple_active_B.euler_tmp_f = 0.0;
    }
    ...
    simple_active_B.ControlSum[simple_active_B.trueCount] = (static_cast<real_T>
      (simple_active_B.value[simple_active_B.trueCount]) *
      simple_active_B.euler_tmp * simple_active_P.PControl_Gain +
      static_cast<real_T>(simple_active_B.value_c[simple_active_B.trueCount]) *
      simple_active_B.dv[simple_active_B.trueCount] *
      simple_active_P.DControl_Gain) + static_cast<real_T>
      (simple_active_B.value_k[simple_active_B.trueCount]) *
      simple_active_B.euler_tmp_f * simple_active_P.IControl_Gain;

    // MATLAB Function: '<S2>/Manage Accumulated Error'
    simple_active_B.CurrentError[simple_active_B.trueCount] =
      simple_active_B.euler_tmp_f;
  }
...
void simple_active::initialize()
{
...

  {
    ...
    static const char_T tmp_0[6] = { 'D', 'G', 'a', 'i', 'n', 's' };

    static const char_T tmp_1[6] = { 'I', 'G', 'a', 'i', 'n', 's' };

    static const char_T tmp_2[16] = { 'r', 'e', 's', 'e', 't', '_', 't', 'h',
      'r', 'e', 's', 'h', 'h', 'o', 'l', 'd' };

    static const char_T tmp_3[18] = { 's', 'c', 'a', 'l', 'i', 'n', 'g', '_',
      'p', 'a', 'r', 'a', 'm', 'e', 't', 'e', 'r', 's' };
    ...
    for (int32_T i = 0; i < 18; i++) {
      simple_active_B.prmName[i] = tmp_3[i];
    }
