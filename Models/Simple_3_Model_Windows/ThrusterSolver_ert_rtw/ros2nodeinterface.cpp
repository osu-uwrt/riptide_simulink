...
// C/C++ source code generated on : Sun Sep 24 14:29:35 2023
//
#ifdef _MSC_VER
#pragma warning(push)
...
#include <chrono>
#include <utility>
const std::string SLROSNodeName("ThrusterSolver");
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
        ...
        std::string NodeName("ThrusterSolver");
        SLROSNodePtr = std::make_shared<rclcpp::Node>(NodeName);
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"** Starting the model \"ThrusterSolver\" **\n");
        mExec = std::make_shared<rclcpp::executors::MultiThreadedExecutor>();
        mExec->add_node(SLROSNodePtr);
        //initialize the model which will initialize the publishers and subscribers
        mModel = std::make_shared<ThrusterSolver>(
        );
		rtmSetErrorStatus(mModel->getRTM(), (NULL));
        mModel->initialize();
        //create the threads for the rates in the Model
        mBaseRateThread = std::make_shared<std::thread>(&NodeInterface::baseRateTask, this);
		mSchedulerTimer = SLROSNodePtr->create_wall_timer(std::chrono::nanoseconds(200000000),std::bind(&NodeInterface::schedulerThreadCallback,this));
    }
